...
//			successRedirect : '/about', 
			...
			failureFlash : true
		}),function(request,response){ console.log(request.user);request.session.user= request.user; response.redirect('/about');});
		...
		async.each(friends,
			...
			console.log('No Friend')
						...
						frdDetails.push(user.user.username+', '+user.user.email +','+user.user.address);
	...
	app.post('/search_member', function(req, res) {
		...
		console.log(regex);
   		...
   		var query = User.find({ $and: [ {'user.username': regex}, { _id: { $ne: req.user._id } } ] } ).limit(20);
      ...
      // Execute query in a callback and return users list
  		...
  		query.exec(function(err, users) {
      		if (!err) {
         		// Method to construct the json result set
            		...
            		//	'Content-Type': 'application/json'
         		//}, 200);
			res.render('search.html', {friends : users});
      		} else {
         		res.send(JSON.stringify(err), {
            			'Content-Type': 'application/json'
         		}, 404);
		...
		app.post('/friend',  function (request, response){
				...
				if(request.param('squery') != ''){
					...
					if (user)
					{ 
						if(user._id == request.param('mainfriendid'))
						{ 
		...
		app.get('/gchat', function(request, response){
				io.sockets.on('connection', function (socket) {
  				 var lineReader = require('readline').createInterface({
    				 input: require('fs').createReadStream("/tmp/test")
   				 });
   				lineReader.on('line', function (data) {
   				//console.log('Line from file:', JSON.parse(data)); 
	
				   socket.emit("serverMsg", JSON.parse(data));
				//send the data to all the clients who are accessing the same site(localhost)
  				 socket.broadcast.emit("serverMsg", JSON.parse(data));
				 });
				});

			response.render('gchat.html',{user : request.user});
		});


		app.get('/resource',auth,function(request, response){
			response.render('resource.html');
		});
...
//send the data to the current client requested/sent.
   ...
   fs.appendFile("/tmp/test", JSON.stringify(data)+"\n", function(err) {
    if(err) {
