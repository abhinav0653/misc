...
namespace SimpleGame {
 public class EnemyManager
  : Microsoft.Xna.Framework.DrawableGameComponent {
   ...
   private readonly float collisionDistance = 64.0 f;
   ...
   public bool EnemiesExist {
    get {
     return (totalEnemies < enemiesThisLevel);
    }
   }
   ...
   public void Load(SpriteBatch spriteBatch, int maxEnemies,
    ...
    for (int i = 0; i < maxEnemies; i++) {
     ...
     enemies[i].Velocity = new Vector2(-speed, 0.0 f);
    }
   }
   ...
   public int CollidedWithPlayer(Vector2 playerPosition) {
    for (int i = 0; i < enemies.Length; i++) {
     ...
     if (enemy.Active) {
      float distance = Vector2.DistanceSquared(playerPosition,
       enemy.Position);
